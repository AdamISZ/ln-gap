...
//! docs/design-notes/SCENARIOS.md. With `--keep`, each scenario's regtest datadir is left
...
fn main() -> anyhow::Result<()> {
    ...
    if filter.is_none() {
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../docs/design-notes/SCENARIOS.md");
        std::fs::write(&path, out)?;
        eprintln!("wrote {}", path.display());
    } else {
        eprintln!("(single scenario: docs/design-notes/SCENARIOS.md left untouched; the report is in regtest-data/<id>/SCENARIO.md with --keep)");
    }

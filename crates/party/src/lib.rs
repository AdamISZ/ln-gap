...
//! knows, and the protocol behaviour for contracts — off-chain negotiation
//! of contract changes, and on-chain reaction to commitments, Moves,
//! deadlines and challenge windows. The same code runs for both roles.
...
use lngap_contract::onchain::{check_claim, decode_claim, parse_move_witness};
use lngap_contract::{Claim, ContractInstance, DepthKeys, Program, CODE_BITS};
use lngap_lamport::keystore::KeyStore;
use lngap_lamport::{uint_to_bits, Reveal};
...
/// negotiation that precedes a channel update.
#[derive(Clone, Debug)]
pub enum PartyMsg {
    Chan(Envelope),
    /// Proposer -> responder: intended change and resulting state with the
    /// proposer's Lamport keys filled in.
    Draft { spec: StateSpec, change: Change },
...
struct Live {
    ...
    tree: TapTree,
    confirmed_at: u32,
    /// State after the last confirmed move (the signed state at depth 0).
    state: Vec<bool>,
    claim_code: Option<u8>,
    /// The current prover's code reveal (the Split leaf re-checks it).
    code_reveal: Option<Reveal>,
    /// The previous prover's state reveal (needed by depth ≥ 2 disproofs).
...
impl Party {
    ...
    pub fn new(
        ...
        Ok(Party {
            ...
            programs: programs.into_iter().map(|p| (p.name().to_string(), p)).collect(),
            expected,
            pending_draft: None,
            pending_change: None,
            waiting_since: None,
    ...
    pub fn program(&self, name: &str) -> Option<&Arc<dyn Program>> {
    ...
    fn peek_move(&self, id: u32) -> Option<Vec<bool>> {
        self.moves.get(&id).and_then(|q| q.front().cloned())
    }
    ...
    pub fn propose_change(&mut self, change: Change) -> Result<Vec<PEnvelope>> {
        ...
        self.say(format!("propose seq {}: {}", spec.seq, describe_change(&change, &self.programs)));
        self.pending_draft = Some(spec.clone());
        self.pending_change = Some((spec.seq, change.clone()));
        self.waiting_since = Some(self.height);
        Ok(vec![self.env(PartyMsg::Draft { spec, change })])
    }
    ...
    /// propose it; if a contract is terminal, propose folding it. Called by
    /// the harness between blocks; returns messages to deliver.
    pub fn poll(&mut self) -> Result<Vec<PEnvelope>> {
        ...
        let contracts: Vec<(u32, Option<Role>)> = self.channel.current_state().contracts.iter().map(|c| {
            let i = downcast(c);
            (i.id, i.turn())
        }).collect();
        for (id, turn) in contracts {
            match turn {
                Some(r) if r == self.role => {
                    if let Some(mv) = self.peek_move(id) {
                        let deadline = self.deadline();
    ...
    pub fn handle(&mut self, env: PEnvelope) -> Result<Vec<PEnvelope>> {
        ...
        let out = match env.msg {
            ...
            PartyMsg::Draft { spec, change } => self.on_draft(spec, change)?,
    ...
    fn on_draft(&mut self, spec: StateSpec, change: Change) -> Result<Vec<PEnvelope>> {
        ...
        match &change {
            Change::Open { deadline, .. } | Change::Move { deadline, .. } => {
                ...
                ensure!(*deadline <= self.height + 2 * p.deadline_offset(), "deadline {deadline} too far");
            }
            _ => {}
        }
        let mut full = spec;
        ...
        *self.expected.lock().unwrap() = Some(state);
        ...
        self.waiting_since = Some(self.height);
        self.say(format!("accept draft seq {seq}: {}", describe_change(&change, &self.programs)));
            ...
            if self.channel.current_seq() >= seq {
                if let Change::Move { id, .. } = change {
                    if self.channel.current_state().contract(id).is_some() {
                        // my move if I was the mover: the queue head was played
    ...
    fn pending_change_mover(&self, change: &Change) -> Option<Role> {
        if let Change::Move { id, .. } = change {
            // the mover was whoever was on turn in the previous state
    ...
    /// Process a confirmed block: channel-level reactions, then contract
    /// reactions, then timers.
    ...
    fn on_commit_confirmed(&mut self, seq: u64, version: Role, height: u32) -> Result<()> {
        ...
        let who = if version == self.role { "my".to_string() } else { "counterparty's".to_string() };
    ...
    fn on_move_confirmed(&mut self, idx: usize, d: u32, tx: &Transaction, height: u32) -> Result<()> {
        ...
        let reveals = parse_move_witness(&tx.input[0].witness, inst.program.n_move_bits(), inst.program.n_state_bits())?;
        ...
        match check_claim(&*inst.program, &claim) {
            Ok(()) => {
                self.say(format!("contract {id}: counterparty's move_{d} is valid"));
                Ok(())
            }
            Err(reason) => {
                self.say(format!("contract {id}: counterparty's move_{d} is INVALID ({reason}); disproving"));
                self.disprove(idx, d, &claim, &reveals)
            }
        }
    }

    fn disprove(&mut self, idx: usize, d: u32, claim: &Claim, reveals: &lngap_contract::onchain::MoveReveals) -> Result<()> {
        let inst = self.instance(&self.live[idx]);
        let specs = inst.disprove_specs(d);
        let Some(spec) = specs.iter().find(|s| (s.detects)(claim)) else {
            self.say(format!("contract {}: no disprove leaf detects the inconsistency — cannot punish", self.live[idx].id));
            return Ok(());
        };
        let l = &self.live[idx];
        ...
        w.push(sig.as_ref().to_vec()).extend(spec.witness_args(l.prior_state_reveal.as_ref(), &reveals.mv, &reveals.state, &reveals.code));
    ...
    fn tick(&mut self) -> Result<()> {
        ...
        if self.channel.closing.is_none() {
            ...
            if self.channel.closing.is_none() {
                let overdue: Vec<u32> = self.channel.current_state().contracts.iter().map(|c| downcast(c)).filter(|i| i.turn().is_some_and(|t| t != self.role) && height >= i.deadline).map(|i| i.id).collect();
                if let Some(id) = overdue.first() {
                    self.say(format!("contract {id}: counterparty missed its deadline; force-closing to settle"));
                    self.channel.force_close()?;
                }
            }
        }
    ...
    fn csv_ready(&self, l: &Live, csv: Option<u16>) -> bool {
        match csv {
    ...
    fn tick_live(&mut self, idx: usize) -> Result<()> {
        ...
        if l.depth == 0 {
            ...
            if my_turn && !self.faults.passive_onchain && self.height < inst.deadline {
                if let Some(mv) = self.peek_move(l.id) {
                    let leaf = l.tree.leaf("move_1")?;
                    if self.csv_ready(&l, leaf.timelock.csv) {
                        return self.broadcast_move(idx, 1, mv);
                    }
                    return Ok(());
                }
            }
            if self.height >= inst.deadline && self.csv_ready(&l, Some(params.to_self_delay)) {
        ...
        if self.csv_ready(&l, Some(window)) {
    ...
    fn broadcast_move(&mut self, idx: usize, d: u32, mv: Vec<bool>) -> Result<()> {
        ...
        let rec = self.channel.record(l.seq).ok_or_else(|| anyhow!("no record"))?;
        let ptx = rec.graph.get(&key).ok_or_else(|| anyhow!("no graph tx {key}"))?;
        let tx = ptx.finalize(&move_witness_args(&mv_r, &st_r, &code_r))?;
        self.pop_move(l.id);
        self.channel.mark_swept(l.outpoint);
...
fn describe_change(c: &Change, programs: &HashMap<String, Arc<dyn Program>>) -> String {
    match c {
        ...
        Change::Move { id, mv, deadline } => {
            let m = programs.values().next().map(|p| p.describe_move_bits(mv)).unwrap_or_default();

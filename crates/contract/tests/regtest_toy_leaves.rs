...
fn coinflip_disprove_leaves_match_native_checks() {
    ...
    let inst = ContractInstance::new(7, prog.clone(), Amount::from_sat(20_000), prog.initial_bits(), 300, vec![k1, k2]).unwrap();
...
fn graph_shape_and_signing() {
    ...
    let inst = ContractInstance::new(1, prog.clone(), Amount::from_sat(20_000), prog.initial_bits(), 300, vec![k1, k2]).unwrap();

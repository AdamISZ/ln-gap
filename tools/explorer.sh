...
# a node already serving this datadir (the demo's venue, an earlier run of
# this script) records its port in <datadir>/rpcport: attach to it and,
# since we did not start it, leave it running on exit
STARTED=0
if [ -f "$DATADIR/rpcport" ] && bitcoin-cli -regtest -datadir="$DATADIR" -rpcport="$(cat "$DATADIR/rpcport")" getblockcount >/dev/null 2>&1; then
  RPC_PORT="$(cat "$DATADIR/rpcport")"
  echo "attaching to the node already serving $DATADIR on rpcport $RPC_PORT (it stays up when this exits)"
elif ! bitcoin-cli -regtest -datadir="$DATADIR" -rpcport="$RPC_PORT" getblockcount >/dev/null 2>&1; then
...
# stop the node only if this script started it
if [ "$STARTED" = 1 ]; then
...
if [ "$STARTED" = 1 ]; then echo "explorer: http://127.0.0.1:$EXPLORER_PORT  (Ctrl-C stops the explorer and this bitcoind)"; else echo "explorer: http://127.0.0.1:$EXPLORER_PORT  (Ctrl-C stops the explorer; the node stays up)"; fi
